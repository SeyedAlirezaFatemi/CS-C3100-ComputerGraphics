    ...
    if (fileName.endsWith(".obj")) {
        ...
        while (std::getline(file, line)) {
            ...
            if (keyword == "obj") {
                ...
                while (std::getline(file, line)) {
                    std::stringstream lightStream(line);
                    lightStream >> keyword;
                    ...
                    if (keyword == "}")
                        ...
                        Vec3f p;
                        lightStream >> p.x >> p.y >> p.z;
                        light.setStaticPosition(p);
                    } else if (keyword == "col") {
                        lightStream >> light.m_color.x >> light.m_color.y >> light.m_color.z;
                    }
                }
            }
        }
...
void App::renderWithNormalMap(GLContext *gl, Mat4f worldToCamera, Mat4f projection) {
    ...
    for (unsigned i = 0; i < m_lights.size(); i++) {
        ...
        String ss = "shadowSampler[";
        ss += ('0' + (char) i);
        ss += ']';
        String ptlc = "posToLightClip[";
        ptlc += ('0' + (char) i);
        ptlc += ']';
...
Image App::convertBumpToObjectSpaceNormal(const Image &bump, MeshBase *pMeshIn, float alpha) {
    ...
    for (int m = 0; m < pMesh->numSubmeshes(); ++m)
        for (int t = 0; t < pMesh->indices(m).getSize(); ++t) {
            ...
            edges[0][0] = v_[3] - v_[1];
            edges[0][1] = -(v_[2] - v_[0]);
            edges[0][2] = -(edges[0][0] * v_[0] + edges[0][1] * v_[1]);
            edges[1][0] = v_[5] - v_[3];
            edges[1][1] = -(v_[4] - v_[2]);
            edges[1][2] = -(edges[1][0] * v_[2] + edges[1][1] * v_[3]);
            edges[2][0] = v_[1] - v_[5];
            edges[2][1] = -(v_[0] - v_[4]);
            edges[2][2] = -(edges[2][0] * v_[4] + edges[2][1] * v_[5]);
