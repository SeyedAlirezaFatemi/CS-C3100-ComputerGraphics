...
bool App::handleEvent(const Window::Event &ev) {
    ...
    if (shading_mode_changed_) {
        common_ctrl_.message(shading_toggle_ ? "Directional light shading using normals; direction to light (0.5, 0.5, -0.6)" : "Joint weight coloring");
        shading_mode_changed_ = false;
    }
...
void App::renderSkeleton() {
    ...
    for (auto i = 0u; i < transforms.size(); ++i) {
        ...
        glColor3f(1, 0, 0); // red
        glVertex3f(joint_world_pos.x, joint_world_pos.y, joint_world_pos.z);
        auto temp = joint_world_pos + scale * right;
        glVertex3f(temp.x, temp.y, temp.z);

        // ..and the y axis.. ("up")
        glColor3f(0, 1, 0); // green
        glVertex3f(joint_world_pos.x, joint_world_pos.y, joint_world_pos.z);
        temp = joint_world_pos + scale * up;
        glVertex3f(temp.x, temp.y, temp.z);

        // ..and the z axis ("ahead").
        glColor3f(0, 0, 1); // blue
        glVertex3f(joint_world_pos.x, joint_world_pos.y, joint_world_pos.z);
        temp = joint_world_pos + scale * ahead;
        glVertex3f(temp.x, temp.y, temp.z);
