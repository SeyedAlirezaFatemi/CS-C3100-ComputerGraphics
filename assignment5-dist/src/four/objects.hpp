...
public:
    Box(const FW::Vec3f &min, const FW::Vec3f &max, Material *m) : Object3D(m), min_(min), max_(max) {
...
public:
    Plane(const FW::Vec3f &normal, float offset, Material *m) : Object3D(m), normal_(normal.normalized()), offset_(offset) {
...
public:
    Sphere(const FW::Vec3f &center, float radius, Material *m) : Object3D(m), center_(center), radius_(radius) {
