...
public:
    ...
    Material(const FW::Vec3f &diffuse_color,
             ...
             const char *texture_filename) : diffuse_color_(diffuse_color),
...
public:
    PhongMaterial(const FW::Vec3f &diffuse_color, const FW::Vec3f &specular_color, float exponent, const FW::Vec3f &reflective_color,
                  const FW::Vec3f &transparent_color, float refraction_index, const char *texture_TGA_filename) : Material(diffuse_color, reflective_color, transparent_color, refraction_index, texture_TGA_filename),
...
public:
    ProceduralMaterial(const FW::Mat4f &matrix, Material *m1, Material *m2) : matrix_(matrix), m1_(m1), m2_(m2) { assert(m1 != nullptr && m2 != nullptr); }
...
public:
    Checkerboard(const FW::Mat4f &matrix, Material *m1, Material *m2) : ProceduralMaterial(matrix, m1, m2) {}
