...
DirectionalLight::DirectionalLight() : direction_(0, 0, 0),
                                       intensity_(1, 1, 1) {}

DirectionalLight::DirectionalLight(const Vec3f &direction, const Vec3f &intensity) : direction_(direction.normalized()), intensity_(intensity) {}
...
PointLight::PointLight(const Vec3f &position, const Vec3f &intensity,
                       ...
                       float quadraticAttenuation) : position_(position),
...
void PointLight::getIncidentIllumination(const Vec3f &p,
    ...
    dir_to_light = FW::normalize(this->position_ - p);
    distance = FW::length(this->position_ - p);
    incident_intensity = this->intensity_ / (this->quadratic_attenuation_ * distance * distance + this->linear_attenuation_ * distance + this->constant_attenuation_);
}
