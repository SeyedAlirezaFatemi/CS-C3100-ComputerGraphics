...
Camera::Camera()
{
    mStartRot.setIdentity();
    mCurrentRot.setIdentity();
}
...
void Camera::ArcBallRotation(int x, int y)
{
    ...
    sz = FW::sqrt(1.0f - sl * sl);
    ez = FW::sqrt(1.0f - el * el);
    ...
    if( dotprod != 1 )
    {
        Vec3f axis( sy * ez - ey * sz, sz * ex - ez * sx, sx * ey - ex * sy );
        axis.normalize();
...
void Camera::PlaneTranslation(int x, int y)
{
    ...
    float d = float(mViewport[3])/2.0f / FW::tan(mPerspective[0]*M_PI / 180.0f / 2.0f);
    ...
    Vec2f move(cr-sr, cu-su);

    // this maps move
    move *= -mCurrentDistance/d;

    mCurrentCenter = mStartCenter +
        + move[0] * Vec3f(mCurrentRot(0,0),mCurrentRot(0,1),mCurrentRot(0,2))
        + move[1] * Vec3f(mCurrentRot(1,0),mCurrentRot(1,1),mCurrentRot(1,2));
...
void Camera::ApplyModelview() const
{
    ...
    // rotate object
    glMultMatrixf(mCurrentRot.getPtr());
...
void Camera::DistanceZoom(int x, int y)
{
    ...
    mCurrentDistance = mStartDistance * FW::exp(delta);  
}
